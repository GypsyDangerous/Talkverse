...
const ConversationHeader = (props) => {
  ...
  return (
    <header className="contact-profile">
      <div className="conversation-header-img">
        <Avatar
          src={recipient?.profilePicture}
          alt={recipient?.name?.toUpperCase()}
        />
      </div>
...
const Conversation = (props) => {
  ...
  useEffect(() => {
    if (props.isNew) {
      const unsubA = firebase.db
        .collection("conversations")
        .onSnapshot(async (snapshot) => {
          const contacts = [].concat.apply(
            [],
            snapshot.docs
              .map((doc) => doc.data())
              .filter((doc) =>
              ...
              .map((conv) => conv.members)
          );
          setContacts(
            !contacts
              ? []
              : contacts.filter((id) => id !== firebase.auth.currentUser.uid)
          );
        });
      ...
      const unsubscribe = firebase.db
        ...
        .onSnapshot(async (snapshot) => {
          try {
            const mine = await snapshot.query
              .where(firebase.documentId(), "==", id)
              .get();
            const minedoc = mine.docs[0];
            const me = { ...minedoc.data(), convid: minedoc.id };
            setConv(me);
            const convs = snapshot.docs.filter(
              (doc) => doc.id === me?.convid
            )[0];
            convs.ref.collection("messages").onSnapshot(async (msgSnapshot) => {
              ...
              const sortedMsgs = sortedData.docs.map((doc) => {
    ...
    <main className="content">
      {!props.empty && !props.isNew && (
        <>
          ...
          <MessageInput
            onSend={() => (contentRef.current.scrollTop += 100000000000000)}
            conversation={conv}
          />
        </>
      )}
          ...
          <div className="is-new">
            <div className="contact-search-bar">
              ...
              <input
                ...
                aria-label="search for new contacts by username or email"
              />
            </div>
              ...
              <ul>
                {!!search &&
                  searchResults
                    .filter((r) => {
                      return (
                        !contacts.includes(r.uid) &&
                        r.uid !== firebase?.auth?.currentUser?.uid &&
                        r.name.toLowerCase().includes(search.toLowerCase())
                      );
                    })
                    .map((s) => (
                      <div
                        onClick={() => createConv(s.uid)}
                        className="search-result"
                      >
                        <div
                          className={`img-container profile-img ${s.status}`}
                        >
